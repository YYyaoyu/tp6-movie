...
class UserInfo extends BaseController
{
    ...
    public function search(){
        ...
        if($_POST['createTime']){
            $result = $this->user
            ->where([
                ['reg_time', '>', $_POST['createTime']],
                ['reg_time', '<', date("Y-m-d",strtotime("+1 day",strtotime($_POST['createTime'])))]
                ])
    ...
    public function login(){
        ...
        if($this->user->where('username', $username)->find()){
            ...
            if($username&&$password){
                $newUser = $this->user->register($username,$password);
                $status = 'success';
                $result = '注册成功'; 
            }
